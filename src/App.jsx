...
export default function App(){
 ...
 {(page==='home')&&(<section className="hero" id="home"><img className="hero-watermark" src={logo} alt="" aria-hidden="true"/><div className="hero-ring" aria-hidden="true"/><div className="hero-content wrap"><p className="eyebrow">BEE BARBER HOUSE <span>✦</span> REAL CUT</p><div className="crest"><img src={logo} alt="Biểu tượng ong cầm tông đơ và kéo của Bee Barber House" width="180" height="164" fetchPriority="high"/></div><h1>Diện mạo chỉn chu.<br/><em>Phong cách riêng.</em></h1><p className="hero-description">Một đường kéo tinh tế. Một mái tóc đúng chất.<br/>Dành thời gian cho bản thân, bắt đầu tại Bee.</p><div className="actions"><a className="button" href="#/lien-he">Ghé Bee Barber House</a><a className="button outline" href="#/dich-vu">Khám phá dịch vụ</a></div></div><div className="hero-bottom wrap"><span>441 VŨ HỮU · THANH XUÂN</span><a href="#/ve-bee">KHÁM PHÁ VỀ BEE</a><span>HÀ NỘI, VIỆT NAM</span></div></section>)}
 {(page==='home'||page==='about')&&(<section className="intro wrap section" id="about"><div><p className="eyebrow">TINH THẦN CỦA BEE</p><h2>Gọn gàng trong diện mạo.<br/><em>Tự tin trong từng ngày.</em></h2></div><div className="intro-copy"><p>Bee Barber House là nơi bạn dành một khoảng thời gian cho chính mình. Chúng tôi lắng nghe, tư vấn và chăm chút từng chi tiết để mái tóc phù hợp với gương mặt, thói quen và cá tính của bạn.</p><a className="text-link" href="#/dich-vu">Tìm kiểu tóc của bạn</a></div></section>)}
 {(page==='home'||page==='services')&&(<section className="service-section section" id="services"><div className="wrap"><div className="section-head"><div><p className="eyebrow">THE BEE SERVICES</p><h2>Chỉn chu từ <em>từng chi tiết.</em></h2></div><span className="side-note">KỸ THUẬT · PHONG CÁCH · TRẢI NGHIỆM</span></div><div className="service-list">{services.map(([n,title,desc,detail])=><a className="service" key={n} href="#/lien-he"><span className="number">{n}</span><h3>{title}</h3><div><p>{desc}</p><small>{detail}</small></div></a>)}</div><p className="service-note">Trao đổi trực tiếp tại quán để được tư vấn dịch vụ và báo giá phù hợp với mái tóc của bạn.</p></div></section>)}
 {(page==='home'||page==='journal')&&(<section className="section wrap" id="journal"><div className="section-head"><div><p className="eyebrow">THE BEE JOURNAL</p><h2>Chuyện tóc. <em>Chuyện phong cách.</em></h2></div><span className="side-note">MỘT CHÚT CHĂM SÓC MỖI NGÀY</span></div><div className="articles">{articles.map((item,i)=><button className="article" key={item.title} onClick={()=>setArticle(item)}><div className={`article-art art-${i}`} aria-hidden="true"><span>BEE / JOURNAL</span><strong>{item.sub}</strong><span className="art-bottom">REAL CUT <span>✦</span> 0{i+1}</span></div><div className="article-copy"><p className="eyebrow">{item.category}</p><h3>{item.title}</h3><span className="read-more">Đọc bài viết</span></div></button>)}</div></section>)}
 {(page==='home'||page==='contact')&&(<section className="contact-section" id="contact"><div className="wrap contact-grid"><div><p className="eyebrow">YOUR NEXT GOOD HAIR DAY</p><h2>Hẹn gặp bạn<br/><em>tại Bee.</em></h2><p>Một mái tóc mới. Một tinh thần mới.<br/>Ghé quán để cùng tìm diện mạo dành riêng cho bạn.</p><a className="button" href={maps} target="_blank" rel="noreferrer">Chỉ đường đến quán</a></div><div className="address-card"><p className="eyebrow">BEE BARBER HOUSE</p><h3>441 Vũ Hữu</h3><p>Thanh Xuân, Hà Nội</p><div className="hours"><span>GIỜ MỞ CỬA</span><strong>09:00 — 20:30</strong></div><p className="visit-note">Bạn có thể ghé trực tiếp trong giờ mở cửa để được tư vấn. Quán hiện chưa nhận đặt lịch qua website.</p><a className="text-link" href={maps} target="_blank" rel="noreferrer">Mở Google Maps</a></div></div></section>)}
